...
export const collectionsIndexPage = () => `
  <section class="page-header">
    ${breadcrumbs([{ label: 'الرئيسية', href: '#/' }, { label: 'كل المجموعات' }])}
    <p class="eyebrow">مختارات سيرين</p>
    <h1>كل المجموعات</h1>
  </section>
    ...
    <div class="collection-grid">
      ${collections.map((c) => `
        <a class="collection-tile" href="#/collections/${c.slug}" style="--tile-image:url('${img(c.cover, { w: 900, h: 1150 })}')">
          <span class="collection-tile-inner"><em>${c.name}</em><small>تسوّقي الآن ${icon('arrow')}</small></span>
        </a>`).join('')}
    </div>
  </section>`
  ...
  return `
    <section class="collection-hero" style="--collection-image:url('${img(collection.cover, { w: 1900, h: 1000 })}')">
      <div class="collection-hero-copy">
        ${breadcrumbs([{ label: 'الرئيسية', href: '#/' }, { label: 'المجموعات', href: '#/collections' }, { label: collection.name }])}
    ...
    <section class="product-section">
      <div class="section-head">
        <div><p class="eyebrow">${items.length} قطع</p><h2>تسوّقي ${collection.name}</h2></div>
      </div>
      <div class="filters" data-sort>
        <button class="active" data-sort-value="default">مختاراتنا</button>
        <button data-sort-value="price-asc">السعر: من الأقل</button>
        <button data-sort-value="price-desc">السعر: من الأعلى</button>
      </div>
