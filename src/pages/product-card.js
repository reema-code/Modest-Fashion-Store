...
export const productCard = (p) => `
  <article class="product" data-collection="${p.collection}">
    <a class="product-image" href="#/product/${p.slug}">
      <img src="${p.thumb}" alt="${p.name}" loading="lazy" decoding="async" width="900" height="1150">
      ${p.badge ? `<span class="badge">${p.badge}</span>` : ''}
      <button class="heart ${wishlistStore.has(p.id) ? 'saved' : ''}" data-wish="${p.id}" aria-label="حفظ ${p.name}">${icon('heart')}</button>
      <span class="quick" data-quick="${p.id}">إضافة سريعة</span>
    </a>
